...
const GooeyNav = ({
  ...
  const updateEffectPosition = (element: HTMLElement | null | undefined) => {
    ...
    try {
      ...
      console.warn('Error updating effect position:', error);
    }
  };

  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
    e.preventDefault();
    const liEl = e.currentTarget.closest('li') as HTMLLIElement | null;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLAnchorElement>, index: number) => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault();
      const liEl = e.currentTarget.closest('li') as HTMLLIElement;
      if (liEl) {
        const clickEvent = {
          currentTarget: liEl,
          preventDefault: () => {}
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (liElements && liElements.length > 0 && activeIndex >= 0 && activeIndex < liElements.length) {
        const activeLi = liElements[activeIndex] as HTMLLIElement;
        if (activeLi) {
